...
<a name="closeAsn"></a>
# **closeAsn**
> List&lt;ProcessOutputAPIModel&gt; closeAsn(body)
...
AsnApi apiInstance = new AsnApi();
ClosePurchProcessInputAPIModel body = new ClosePurchProcessInputAPIModel(); // ClosePurchProcessInputAPIModel | Input data for ClosePurchProcess process.
try {
    List<ProcessOutputAPIModel> result = apiInstance.closeAsn(body);
    System.out.println(result);
} catch (ApiException e) {
    System.err.println("Exception when calling AsnApi#closeAsn");
    e.printStackTrace();
}
```
